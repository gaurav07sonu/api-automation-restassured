...
public class DocumentSearch extends APIDriver {
	...
	public void fetchsearch(String ticker, String query) throws CoreCommonException {
		try {
			...
			verify.verifyEquals(respJson.getJSONObject("response").getBoolean("status"), true,
				...
				boolean tickerFlag = true;
				StringBuffer sb = new StringBuffer();
				if (total_results != 0) {
					if (documentResults_size.length() != 0) {
						for (int i = 0; i < documentResults_size.length(); i++) {
							String ticker1 = documentResults_size.getJSONObject(i).getString("ticker");
							String id = documentResults_size.getJSONObject(i).getJSONObject("doc_source").getString("id");
							if(!ticker.equalsIgnoreCase(ticker1)) {
								tickerFlag = false;
								sb.append(" " + id + " ");
							}
							//verify.assertEqualsActualContainsExpected(ticker1, ticker, "ticker equals");
						}
					}
				}
				verify.assertTrue(tickerFlag, "Checking if ticker value: " + ticker + " in docs data is correct " + sb.toString());
			} 
	...
	@Test(groups = "sanity", description = "applying new doc type filter", dataProvider = "test_newdoctype_Filter", dataProviderClass = DataProviderClass.class)
	public void fetchsearch_newdoctype(String ticker, String filters) throws CoreCommonException {
		...
		try {
			...
			verify.assertTrue(tickerFlag,
					"Checking if ticker value: " + ticker + " in docs data is correct " + sb.toString());
...
@Test(groups = "sanity", description = "doc type and custom date as filter combinations", dataProvider = "doctype_customdate_filters_combination", dataProviderClass = DataProviderClass.class)
	public void docsearch_customdate_filter(String ticker, String date_range_from, String date_range_to, String filters) throws CoreCommonException {
	...
	public void docsearch_language_filter(String ticker, String language, String filters) throws CoreCommonException {
	...
	public void docsearch_context_filter(String ticker, String sections_filter, String filters) throws CoreCommonException {
