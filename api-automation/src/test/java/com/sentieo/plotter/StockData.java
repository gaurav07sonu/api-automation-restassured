					...
					if (apiResp.getStatusCode() == 200) {
						...
						if (intraday.length() != 0) {
							...
							verify.verifyEquals(respJsonStock.getJSONObject("response").getBoolean("status"), true,
									"Verify the API Response Status");
							verify.verifyResponseTime(resp, 5000);
							JSONArray values = respJsonStock.getJSONObject("result").getJSONArray("series")
							...
							verify.assertEqualsActualContainsExpected(current_StockDate, stock_Date,
									"Verify Stock dates");
							verify.verifyEquals(current_StockValue, plotter_StockValue, "Verify stock values ");
						} else
							verify.assertTrue(false,
									"verify that intradya data is not blank" + "Intraday size is " + intraday.length());
					}
				}
			}
